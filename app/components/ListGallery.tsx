...
import { deleteData, getGallery } from "@/lib/supabase/supabaseHelper";
import { supabase } from "@/lib/supabase/supabaseClient";
import { Delete, Edit } from "@/public/icons/iconSets";

interface Prop {
  admin: boolean;
  sendToParent?: (gallery: Gallery) => void;
}
...
export default function ListGallery({ admin, sendToParent = () => {} }: Prop) {
  const adminStatus = admin;

  const [data, setData] = useState<Gallery[]>([]);
  const [loading, setLoading] = useState(true);

  const alam = "Keindahan Alam Maluku Utara";
  const kegiatan = "Kegiatan DKP Maluku Utara";
  ...
  if (!data.length) return <p>Belum ada data galeri terdaftar</p>;
                    ...
                    >
                      ...
                      >
                        ...
                        <div
                          ...
                          onClick={async () => {
                            try {
                              await deleteData("gallery", e.id);
                              alert("SUCCESSFULLY to delete gallery entry.");
                            } catch (err) {
                              console.error(err);
                              alert("Failed to delete gallery entry.");
                            }
                          }}
                        >
