...
export default function DashboardWorkflowView({ stage, onExit, workflowId: suppliedWorkflowId, detailPath, action = "list", onUploadReadinessChange }: Props) {
  ...
  const { approvals, approvalMessages, refreshApprovals } = usePublicationApprovals("dataset", workflowId);
  ...
  const [, setSourceUploads] = useState<DashboardSourceUpload[]>([]);
  ...
  const submitPublication = async () => { if (!workflow || !allUploadsSaved || !allVisualizationsSaved) return; setBusy(true); const { error } = await supabase.from("datasets").update({ published: "requested", column_config: { dashboardWorkflow: { selectedTabs: selected, currentStage: "publication", activeTab, uploadStatus: workflow.upload_status, visualizationStatus: workflow.visualization_status, sourcePartnerId } }, published_config: { dashboard: { selectedTabs: selected } } }).eq("id", workflow.id); const reviewResult = error ? null : await supabase.rpc("request_publication_review", { p_resource_kind: "dataset", p_resource_id: workflow.id }); const finalError = error ?? reviewResult?.error; setBusy(false); setMessage(finalError ? finalError.message : "Dashboard dikirim untuk persetujuan publikasi."); if (!finalError) await Promise.all([loadWorkflow(workflow.id), refreshApprovals()]); };
  const updatePublicationStatus = async (status: "pending" | "approved" | "rejected", reviewMessage: string) => { if (!workflow || !isApprovalRole(role)) return; setBusy(true); const { error } = await supabase.rpc("set_publication_approval", { p_resource_kind: "dataset", p_resource_id: workflow.id, p_status: status, p_message: reviewMessage }); setBusy(false); setMessage(error ? error.message : "Status persetujuan diperbarui."); if (error) throw error; await Promise.all([loadWorkflow(workflow.id), refreshApprovals()]); };
  ...
  return <section className="space-y-5">{tabs}<div className="rounded-2xl border bg-white p-5 shadow-md"><h2 className="text-lg font-bold">Publikasi Dashboard Perikanan</h2>{selected.map((tab) => <article key={tab} className="mt-3 rounded-xl border p-4"><strong>{DASHBOARD_CONFIG[tab].label}</strong><p className="text-sm text-stone-600">{DASHBOARD_CONFIG[tab].description}</p></article>)}<p className="mt-4 text-sm">Status: {workflow.publication_status ?? "draft"}</p>{workflow.publication_status && <div className="mt-3"><PublicationApprovalChips approvals={approvals} messages={approvalMessages} /></div>}</div>{isApprovalRole(role) && workflow.publication_status && <PublicationReviewControls role={role} currentStatus={approvals[role]} disabled={busy} onDecision={updatePublicationStatus} />}{workflow.publication_status === null && <Button disabled={!allUploadsSaved || !allVisualizationsSaved || busy} loading={busy} onClick={submitPublication}>Kirim untuk Publikasi</Button>}{message && <p className="rounded-xl bg-sky-50 p-3 text-sm">{message}</p>}</section>;
}
